...
public class MatrixUtilsTest extends TestBase {
  ...
  public void testSumRows() {
    assertThat(MatrixUtils.sumRows(nonZeroColumnMatrix(), null))
        .isEqualTo(new double[] {11, 21.5, 3});
  }
  ...
  public void testMaxInColumns() {
    final double[] expectedMaxValues = new double[] {0.00, 5.00, 5.00, 19.00};

    final int[] expectedMaxIndices = new int[] {1, 2, 2, 1};

    final double[] actualMaxValues = new double[4];
    final int[] actualMaxColumnIndices =
        MatrixUtils.maxInColumns(nonZeroColumnMatrix(), null, actualMaxValues);
  ...
  public void sortedRowsView() {
    final DenseDoubleMatrix2D input =
        new DenseDoubleMatrix2D(
            new double[][] {
              {1, -1},
              {2, -2},
  ...
  private void checkL1Normalization(DoubleMatrix2D unnormalisedColumns) {
    for (int c = 0; c < unnormalisedColumns.columns(); c++) {
      ...
      if (length != 0) {
        assertThat(length).as("Column " + c + "length").isEqualTo(1.0, Offset.offset(0.02));
      }
    }
  }
